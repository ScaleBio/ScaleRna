...
if( params$is_comparison ){
    ...
    rownames(sample_stats) <- sample_stats[["Metrics"]]
}
...
if(params$is_comparison){
    ...
    mito_thresh <- as.numeric(sample_stats[Metric == "maximum_mito_pct", 3])
    gene_thresh <- as.numeric(sample_stats[Metric == "maximum_gene_count", 3])
    umi_thresh <- as.numeric(sample_stats[Metric == "maximum_umi_count", 3])
}

clusterDat[["outlier_classification"]] <- gsub(pattern = "cell;", x = clusterDat[["classification"]], replacement = "")
clusterDat[["outlier_classification"]] <- gsub(pattern = "mitochondrial read percentage", x = clusterDat[["outlier_classification"]], replacement = "mito %")
clusterDat[["outlier_classification"]] <- gsub(pattern = "cell", x = clusterDat[["outlier_classification"]], replacement = " cell")
...
plotUMAP <- function(dat, xVar, yVar, colorVar, sketched, colorPal = "None"){
...
plotStackedBar <- function(dat, groupVar, datVar, colorPal = "None"){
...
plotViolin(dat = clusterDat, groupVar = clusterColumn, datVar = "umis", mad_threshold = umi_thresh) + legend_theme + guide_opts_vln + labs(fill = "Cluster") + xlab("Cluster") + ylab("# of UMIs") + labs(title = "Number of UMIs per Cluster")
```

```{r vlnPlotGenesCluster, eval=seuratPlots}
plotViolin(dat = clusterDat, groupVar = clusterColumn, datVar = "genes", mad_threshold = gene_thresh) + legend_theme + guide_opts_vln + labs(fill = "Cluster") + xlab("Cluster") + ylab("# of Genes") + labs(title = "Number of Genes per Cluster")
```
...
```{r umapOutliers}
plotUMAP(dat = clusterDat, xVar = umap_X, yVar = umap_Y, colorVar = "outlier_classification", sketched = is_sketch) + legend_theme + guide_opts_umap + labs(color = "Outlier Class.") + labs(title = "UMAP Colored by Outlier Classification")
```
...
```{r umapRTbarcode}
plotUMAP(dat = clusterDat, xVar = umap_X, yVar = umap_Y, colorVar = "RT_alias", sketched = is_sketch) + legend_theme + guides(color = guide_legend(override.aes = list(shape = 20, size = 1, alpha = 1), nrow = 32)) + labs(color = "RT Well") + labs(title = "UMAP Colored by RT Well")
```

```{r umapUmis}
plotUMAP(dat = clusterDat, xVar = umap_X, yVar = umap_Y, colorVar = "umis", sketched = is_sketch) + legend_theme + labs(color = "# of UMIs") + scale_color_continuous(trans = "log", labels = scales::label_number(big.mark = ",")) + labs(title = "UMAP Colored by Number of UMIs")
```
...
```{r umapReads}
plotUMAP(dat = clusterDat, xVar = umap_X, yVar = umap_Y, colorVar = "reads", sketched = is_sketch) + legend_theme + labs(color = "# of Reads") + scale_color_continuous(trans = "log", labels = scales::label_number(big.mark = ",")) + labs(title = "UMAP Colored by Number Of Reads")
```
...
```{r stackedBarRT, eval=seuratPlots}
plotStackedBar(dat = clusterDat, groupVar = clusterColumn, datVar = "RT_alias") + legend_theme + guides(fill = guide_legend(override.aes = list(shape = 21, size = 1, alpha = 1), nrow = 32)) + labs(fill = "RT Well") + xlab("Cluster") + ylab("RT Well Proportion") + labs(title = "RT Well Proportion per Seurat Cluster")
```
...
plotStackedBar(dat = clusterDat, groupVar = clusterColumn, datVar = "outlier_classification") + legend_theme + guide_opts_bar + labs(fill = "Outlier Classification") + xlab("Cluster") + ylab("Proportion") + labs(title = "Outlier Proportion per Seurat Cluster")
```
...
tabDat <- clusterDat %>% group_by(sample, classification) %>% summarise(count = n()) %>% mutate(percent = count / sum(count))
