...
process SampleReportGeneration {
	...
	input:
	tuple val(id), val(libName), path("${id}_metrics/allCells.csv"), path("${id}_metrics/allBarcodes.parquet"), val(sampleInfo), val(libCount), path("${id}_metrics/sample_stats.csv"), path("Log.final.out"), path("ScalePlex.allBarcodes.parquet"), path("scaleplex_stats.csv"), path("metrics.csv"), val(hashLibName)
	val(libStructName)
	path(libStructDir)
	val(isBarnyard)
	...
	if (params.scalePlex && hashLibName) {
	...
	--workflowVersion $workflow.manifest.version --starLog Log.final.out $opts
	"""
}
...
take:
	...
	soloLog // STAR Log.final.out files
...
main:
	...
	// sampleMetrics -> [id, libName, allCells.csv, allBarcodes.parquet, sampleInfo, libCount, starLog)
	allCells
	...
	if (params.scalePlex) {
		sampleMetrics
			.join(hashResults, remainder: true) // [rnaId, cellMetrics, scaleplex_stats.csv, metrics.csv, hashLibName]
			.map{ it ->
				// last element null indicates hashResults is empty
				if (it[-1] == null) {
	...
	SampleReportGeneration(sampleMetrics, libJson.getName(), libJson.getParent(), isBarnyard, outputDirForReport, mergedSamples)
